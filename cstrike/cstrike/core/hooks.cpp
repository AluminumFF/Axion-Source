...
bool H::Setup()
{
	...
	SilentInput.HookIndex((int)VTABLE::CLIENT::PREDICTION, PredictionSimulation);
...
void* CS_FASTCALL H::InputParser(CCSInputMessage* Input, CCSGOInputHistoryEntryPB* InputHistoryEntry, char a3, void* a4, void* a5, void* a6)
{
	...
	if (I::Engine->IsConnected() || I::Engine->IsInGame() && SDK::LocalPawn && SDK::LocalPawn->GetHealth() > 0 ) {
		...
		// start properly lagcomp 
		if (F::LAGCOMP::lagcomp->lag_comp_sub_tick.msg == F::LAGCOMP::impl::msg_t::lag_compensation) {
			if (InputHistoryEntry && InputHistoryEntry->m_sv_interp0 && InputHistoryEntry->m_sv_interp1) {
				auto best_tick = F::LAGCOMP::lagcomp->lag_comp_sub_tick.best_tick;
				auto best_tick_fraction = F::LAGCOMP::lagcomp->lag_comp_sub_tick.best_tick_fraction;
				InputHistoryEntry->m_cl_interp->m_nSrcTick = best_tick;
				InputHistoryEntry->m_cl_interp->m_nDstTick = best_tick + 1;
				InputHistoryEntry->m_cl_interp->m_flFraction = best_tick_fraction;

				InputHistoryEntry->m_sv_interp0->m_nSrcTick = best_tick;
				InputHistoryEntry->m_sv_interp0->m_nDstTick = best_tick + 1;
				InputHistoryEntry->m_sv_interp0->m_flFraction = 0.f;

				InputHistoryEntry->m_sv_interp1->m_nSrcTick = best_tick + 1;
				InputHistoryEntry->m_sv_interp1->m_nDstTick = best_tick + 2;
				InputHistoryEntry->m_sv_interp1->m_flFraction = 0.f;
			}
		}
		
		if (C_GET(bool, Vars.legit_enable)) {

			if (F::LEGIT::legit->sub_tickdata.command == F::LEGIT::impl::commands_msg::viewangles) {
				InputHistoryEntry->m_pViewCmd->m_angValue = F::LEGIT::legit->sub_tickdata.best_point;
				InputHistoryEntry->m_pViewCmd->m_angValue.Clamp();
				F::LEGIT::legit->sub_tickdata.response = F::LEGIT::impl::responses_msg::validated_view_angle;

			}
			else
				F::LEGIT::legit->sub_tickdata.response = F::LEGIT::impl::responses_msg::empty;
		}
		else {
			...
			if (F::RAGE::rage->rage_data.rapid_fire) {
				if (rage->sub_tick_data.command == (impl::command_msg::rapid_fire)) {
					...
					// if user's scoped force unzoom for better rapid fire performance
					if (SDK::LocalPawn->IsScoped())
						SDK::Cmd->m_nButtons.m_nValue &= ~IN_SECOND_ATTACK;
...
float CS_FASTCALL H::GetRenderFov(uintptr_t rcx)
{
	...
	else
	{
		...
		if (C_GET(bool, Vars.bFOV)) {
			// if scoped
			if (localPlayerPawn->IsScoped() ){
				// only run if we force on zoom since we scoping
				if (C_GET(bool, Vars.bFOVScoped)) 
					return C_GET(float, Vars.fFOVAmount);
				else // otherwise return original fov 
					oFOV(rcx);
			}
			else 
				return C_GET(float, Vars.fFOVAmount);	
...
float CS_FASTCALL H::SetViewModelFOV()
{
	...
	else
	{
		...
		if (C_GET(bool, Vars.bSetViewModelFOV)) {
			if (localPlayerPawn->IsScoped()) {
				if (C_GET(bool, Vars.bSetViewModelFOVScoped))
...
void CS_FASTCALL H::HandleGameEvents(void* rcx, IGameEvent* const ev)
{
	...
	case FNV1A::HashConst(CS_XOR("player_death")): {
		...
		auto local_idx = controller->GetIdentity()->GetIndex();

		if (event_controller->GetIdentity()->GetIndex() == local_idx) {

			F::LEGIT::legit->Events(ev, F::LEGIT::impl::events::player_death);
			just_died = true;
		}
		else if (event_attacker->GetIdentity()->GetIndex() == controller->GetIdentity()->GetIndex() && event_controller->GetIdentity()->GetIndex() != local_idx) {
	...
	case FNV1A::HashConst(CS_XOR("round_start")): {
		...
		F::LEGIT::legit->Events(ev, F::LEGIT::impl::events::round_start);
...
void H::AllowCameraAngleChange(CCSGOInput* pCSGOInput, int a2)
{
	...
	pCmd->m_csgoUserCmd.m_pBaseCmd->m_pViewangles->m_angValue = F::MISC::MOVEMENT::movement->_move_data.angStoredView;
...
bool CS_FASTCALL H::CreateMove(CCSGOInput* pInput, int nSlot, bool nUnk, std::byte nUnk2)
{
	...
	if (SDK::LocalPawn->GetHealth() > 0) {
		...
		cheat->tickbase = SDK::LocalController->m_nTickBase();
		cheat->canShot = SDK::LocalPawn->CanShoot(I::GlobalVars->nTickCount);
	}
	if (cheat->alive) {
		// we previously died we will respawn
		// update skins & gloves
		if (just_died) {
			Vars.should_update = true;
			Vars.full_update = true;
		...
		F::MISC::MOVEMENT::movement->_move_data.angStoredView = pCmd->m_csgoUserCmd.m_pBaseCmd->m_pViewangles->m_angValue;
		...
		*/
		...
		//if (C_GET(bool, Vars.rage_enable) || C_GET(bool, Vars.legit_enable))
		//	F::LAGCOMP::lagcomp->Start(pCmd);

		// process legit bot w predicted viewangles
		if (C_GET(bool, Vars.legit_enable)) {
			F::LEGIT::legit->SetupAdaptiveWeapon(SDK::LocalPawn);
			F::LEGIT::legit->Run(SDK::LocalPawn, pInput, pCmd);
		}
		...
		if (C_GET(bool, Vars.rage_enable)) {
			// removed temporary but is working just not listed on yg

			F::RAGE::rage->SetupAdaptiveWeapon(SDK::LocalPawn);
			F::RAGE::rage->Run(SDK::LocalPawn, pInput, pCmd);
		}

		F::MISC::MOVEMENT::movement->ProcessMovement(pCmd, SDK::LocalController, SDK::LocalPawn);
	}
...
void CS_FASTCALL H::OverrideView(void* pClientModeCSNormal, CViewSetup* pSetup)
{
	...
	auto thirdperson = C_GET(KeyBind_t, Vars.thirdperson_key, 0);

	auto active = IPT::GetBindState(&thirdperson) && C_GET(bool, Vars.bThirdperson);
	if (active)
	{
