...
static constexpr const char* arrAA[] = {
	"None",
	"Backwards",
	"Forward",
};
static constexpr const char* arrPitch[] = {
	"None",
	"90",
	"-90",
	"Zero"
	...
	if (ImGui::Begin(CS_XOR("handle0000"), &bMainWindowOpened, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBringToFrontOnFocus)) {
		...
		ImGui::BeginGroup();
		{
			std::vector<std::vector<std::string>> tab_columns = {
				{ "c" ,"c", "b", "f", "o", "e" ,"g" },
				{ "Legitbot", "Ragebot", "Antiaim", "Players", "Visuals", "Misc", "Skins" },
				{ "Aims in a legit way at targets...", "Aims agressively at targets...",  "Accuracy assistance...", "Player visualisation", "Visuals", "Engine, others..", "Inventory customization..." },
				{  "lol", "Have you switched to the Aimbot tab? You're just crazy!", "Have you switched to the Visuals tab, do you want to get banned?", "Switched to visuals tabidk", "You switched to the skins tab, why do you need self-deception??", "You switched over.. And yes, to hell with it, come up with a script yourself.", "You switched over.. And yes, to hell with it, come up with a script yourself." }
			};
		...
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, tab_alpha * style.Alpha);
		{
			if (active_tab == legit_tab)
			{
				edited::BeginChild(CS_XOR("##Container0"), ImVec2((c::background::size.x - 200) / 2, c::background::size.y), NULL);
				{
					...
					// run legitr cfg depending on weapon
					{
						ImGui::TextColored(ImColor(ImGui::GetColorU32(c::elements::text)), "General");
						edited::Checkbox("Enabled", "Activate ragebot", &C_GET(bool, Vars.legit_enable));
						ImGui::SliderFloat(CS_XOR("FOV"), &C_GET_ARRAY(float, 7, Vars.legit_fov, current_weapon), 0, 180);
						edited::Checkbox("Perfect silent", "Only works on a 45 below angle degree", &C_GET_ARRAY(bool, 7, Vars.perfect_silent, current_weapon));

						const char* targets_select[3]{ CS_XOR("Crosshair"), CS_XOR("Damage"),CS_XOR("Distance") };
						edited::Combo(CS_XOR("Target selection"), CS_XOR("Select target based on conditions"), &C_GET_ARRAY(int, 7, Vars.legit_target_select, current_weapon), targets_select, IM_ARRAYSIZE(targets_select), 6);
						edited::Keybind("Keybind", "", &C_GET(KeyBind_t, Vars.legit_key), &C_GET(int, Vars.legit_key_style) );
						
						edited::Checkbox("Recoil control", "Controls weapon recoil accuracy", &C_GET_ARRAY(bool, 7, Vars.legit_rcs, current_weapon));

						ImGui::TextColored(ImColor(ImGui::GetColorU32(c::elements::text)), "Accuracy");

						edited::Checkbox("Smoothing","Makes a way to smooth/optimize aiming", & C_GET_ARRAY(bool, 7, Vars.legit_smoothing, current_weapon));
						if (C_GET_ARRAY(bool, 7, Vars.legit_smoothing, current_weapon))
							ImGui::SliderInt(CS_XOR("Smoothness"), &C_GET_ARRAY(int, 7, Vars.legit_smooth, current_weapon), 1, 10);

						edited::Checkbox("Triggerbot", "Auto fires at target when he's inside crosshair", &C_GET_ARRAY(bool, 7, Vars.legit_triggerbot, current_weapon));
					}
				}
				edited::EndChild();
				ImGui::SameLine(0, 0);
			...
			else if (active_tab == rage_tab)
			{
				edited::BeginChild(CS_XOR("##Container0"), ImVec2((c::background::size.x - 200) / 2, c::background::size.y), NULL);
				{
					...
					// run rage cfg depending on weapon
					{
						...
						const char* silent_mode[2]{ CS_XOR("Always"), CS_XOR("Prefer") };
						edited::Combo(CS_XOR("Silent mode"), CS_XOR("Determine if should aim only with sub ticks angles ( avoid getting onshot )"), &C_GET_ARRAY(int, 7, Vars.rage_silent_mode, current_weapon), silent_mode, IM_ARRAYSIZE(silent_mode), 6);
			...
			else if (active_tab == antiaim_tab)
			{
			...
			else if (active_tab == players_tab) {
				edited::BeginChild("##Container0", ImVec2((c::background::size.x - 200) / 2, c::background::size.y), NULL);
				{
					...
					edited::Checkbox(CS_XOR("Backtrack Skeleton"), CS_XOR("Shows player lag compensated bones as skeleton"), &C_GET(bool, Vars.bSkeletonLagComp));
					if (C_GET(bool, Vars.bSkeletonLagComp))
						edited::Color(CS_XOR("##chamscolor"), CS_XOR("Change Backtrack Skeleton color"), &C_GET(ColorPickerVar_t, Vars.colSkeletonBacktrack).colValue, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreviewHalf);
			...
			else if (active_tab == visuals_tab) {

				edited::BeginChild("##Container0", ImVec2((c::background::size.x - 200) / 2, c::background::size.y), NULL);
				{

					ImGui::TextColored(ImColor(ImGui::GetColorU32(c::elements::text)), "Players");
					edited::Checkbox(CS_XOR("Thirdperson"), CS_XOR("Enables thirdperson view"), &C_GET(bool, Vars.bThirdperson));
					if (C_GET(bool, Vars.bThirdperson)) {
						edited::Keybind("Key", "For thirdperson activaion", &C_GET(KeyBind_t, Vars.thirdperson_key, 0), 0);
						ImGui::SliderFloat(CS_XOR("Distance"), &C_GET(float, Vars.flThirdperson), 0.f, 150.f);
					}

					edited::Checkbox(CS_XOR("Field Of View changer"), CS_XOR("Changes your fov"), &C_GET(bool, Vars.bFOV));
					if (C_GET(bool, Vars.bFOV)) {
						edited::Checkbox(CS_XOR("Force on scope"), CS_XOR("Ignore if scoped"), &C_GET(bool, Vars.bFOVScoped));
						ImGui::SliderFloat(CS_XOR("Amount"), &C_GET(float, Vars.fFOVAmount), 30.f, 150.f);
					}

					edited::Checkbox(CS_XOR("Viewmodel changer"), CS_XOR("Change your viewmodel fov"), &C_GET(bool, Vars.bSetViewModelFOV));
					if (C_GET(bool, Vars.bSetViewModelFOV))
					{
						edited::Checkbox(CS_XOR("Force on scope"), CS_XOR("Ignore if scoped"), &C_GET(bool, Vars.bSetViewModelFOVScoped));

						ImGui::SliderFloat(CS_XOR("Amount"), &C_GET(float, Vars.flSetViewModelFOV), 40.f, 150.f);
				...
				edited::BeginChild("##Container0", ImVec2((c::background::size.x - 200) / 2, c::background::size.y), NULL);
				{
					...
					edited::Checkbox(CS_XOR("Strafer asstiance"), CS_XOR("Compensate speed for better strafing"), &C_GET(bool, Vars.bAutostrafeAssistance));
					if (C_GET(bool, Vars.bAutostrafeAssistance))
						ImGui::SliderFloat(CS_XOR("Smoothing"), &C_GET(float, Vars.autostrafe_smooth), 0.f, 100.f);
				...
				{					
					...
					if (!vecDumpedItems.empty()) {
						...
						if (pSelectedItem) {
							...
							//	ImGui::Image((void*)pSelectedItem->m_textureID, ImVec2(50, 90));
