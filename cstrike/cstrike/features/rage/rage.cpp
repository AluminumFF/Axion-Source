...
public:
    ...
    scanned_target(C_BaseEntity* ent, Vector_t eyepos, Vector_t end_pos, C_CSPlayerPawn* record, const scan_data& data) //-V688 //-V818
    {
        this->ent = ent;
        this->record = record;
        this->data = data;
...
float get_las_valid_sim_time() noexcept {
    ...
    const float max_delta = std::min((sv_maxunlag - correct), max_ms / 100.f);
...
public:
    ...
    backtrack_entity(CBaseHandle handle) noexcept : handle(handle) {
        CS_ASSERT(I::GlobalVars != nullptr);

        auto max_unlag = I::Cvar->Find(FNV1A::Hash(CS_XOR("sv_maxunlag")))->value.fl;
        auto interval = I::GlobalVars->flIntervalPerTick;
        auto max_ticks = static_cast<int>(max_unlag / interval);
        ...
        records.resize(records.capacity()); 
    }
    ...
    void save_data(unsigned int index) noexcept {
        ...
        auto controller = I::GameResourceService->pGameEntitySystem->Get<CCSPlayerController>(handle);
        if (!SDK::LocalPawn) {
            invalidate();
            return;
        }
        
        if (!controller || controller->IsLocalPlayerController() || !controller->IsPawnAlive()) {
            invalidate();
            return;
        }

        auto player = I::GameResourceService->pGameEntitySystem->Get<C_CSPlayerPawn>(controller->GetPawnHandle());
        if (!player) {
            invalidate();
            return;
        }
        ...
        const auto skeleton = game_scene_node->GetSkeletonInstance();
        if (!skeleton) {
            invalidate();
            return;
        }
...
void F::LAGCOMP::impl::Render() noexcept {
    ...
    for (const auto& [index, entity] : records) {
        ...
        for (uint32_t i = 0; i < entity->num_bones; i++) {
            ...
            ImGui::GetBackgroundDrawList()->AddLine(ImVec2(screen_pos.x, screen_pos.y), ImVec2(screen_parent_pos.x, screen_parent_pos.y), C_GET(ColorPickerVar_t, Vars.colSkeletonBacktrack).colValue.GetU32(), 2.f);
        }
    }
}
void F::LAGCOMP::impl::Start(CUserCmd* cmd) noexcept {
    ...
    // keep it on extrapolation as we aint using lagcomp
    lag_comp_sub_tick.reset();
    ...
    // call lagcomp from InputHistory hook in order to fix ton of issues not needed to mention here
    lag_comp_sub_tick.msg = msg_t::lag_compensation;
}
...
void F::RAGE::impl::ScanPoint(C_CSPlayerPawn* pLocal, CUserCmd* cmd, C_CSPlayerPawn* record, scan_data& data, bool optimized) {
    if (!pLocal || !record)
        return;

    if (pLocal->GetHealth() <= 0 || record->GetHealth() <= 0)
        return;
    ...
    {
        float hitbox_scale = {}; QAngle_t temp_angle = {};  Vector_t point_pos = {};  Vector4D_t hitbox_rot = {}; QAngle_t point_angle = {}; QAngle_t angle = {};

        // create our ange based on our current point
        record->CalculateHitboxData(hitbox, point_pos, hitbox_rot, hitbox_scale, true);
        MATH::VectorAngless(pLocal->GetEyePosition() - point_pos, point_angle);

        // shit got in vec convert to a quarention angle
        auto vec = get_target_angle(pLocal, point_pos);
        angle.ToVec(vec);

        damage = rage->ScaleDamage(record, pLocal, weapon, point_pos, damage, can_hit);

        points.emplace_back(scan_point(point_pos, point_angle, angle, hitbox, center, damage, can_hit));
    }
    ...
    auto hp = record->GetHealth();

    auto minimum_damage = rage_data.minimum_damage;
    float current_minimum_damage = minimum_damage;
    if (current_minimum_damage > hp)
        current_minimum_damage = hp + 5.f;

    auto best_damage = 0.f;
    scan_point* best_point = nullptr;
    ...
    for (auto& point : points) {
        ...
        if (!(point.hitbox == HEAD) && point.damage > hp + 2.f )
        {
    ...
    if (best_point && best_point->damage >= current_minimum_damage) {

        data.point = *best_point;
        data.damage = best_point->damage;
        data.hitbox = best_point->hitbox;
    }
}
...
void F::RAGE::impl::ScanTarget(C_CSPlayerPawn* pLocal, CUserCmd* cmd, QAngle_t viewangles)
{
    ...
    for (auto& target : targets)
    {
        ...
        rage->ScanPoint(pLocal, cmd, player, last_data, true);

        scanned_targets.emplace_back(scanned_target(target.entHandle, pLocal->GetEyePosition(), player->GetEyePosition(), player, last_data));
    }
}
...
static bool compare_targets(const scanned_target& first, const scanned_target& second)
{
    ...
    if (first.data.damage > second.data.damage)
        return true;
    else if (first.data.damage < second.data.damage)
        return false;
...
void F::RAGE::impl::SortTarget()
{
    auto validate_target = [&](scanned_target* a, scanned_target* b) -> bool {
        ...
        if (a->data.damage == b->data.damage)
...
void F::RAGE::impl::Run(C_CSPlayerPawn* pLocal, CCSGOInput* pInput, CUserCmd* cmd) {
    ...
    bool cfg_auto_scope = true;//TODO: rage_data.auto_scope;
    ...
    auto pred_punch = cache.m_Size > 0 ? cache.m_Data[cache.m_Size - 1] : QAngle_t(0, 0, 0);
    ...
    if (cheat->canShot) {
        if (rage_data.rapid_fire && rage_data.accurate) {
            cmd->AdjustAttackStartIndex(pCmd->m_nTickCount);
        ...
        else
        {
            if (rage_data.accurate) {
                pCmd->m_nTickCount = best_tick_time;
                cmd->m_nButtons.m_nValue &= ~IN_USE;
                ...
                // no silent mode
                if (rage_data.rage_silent_mode == silent_mode::prefer) {
                    pInput->angViewAngles = best_point; pInput->angViewAngles.Clamp();
                    // ensure our viewangles are locked at bestpoint bcuz delay
                    if ( pInput->angViewAngles == best_point) 
                        cmd->m_nButtons.m_nValue |= IN_ATTACK;              
                }
                else// our InputMessage set our predicted subtick viewangles to best point we ready to shoot
                         cmd->m_nButtons.m_nValue |= IN_ATTACK;
            }
...
void F::RAGE::impl::SetupAdaptiveWeapon(C_CSPlayerPawn* pLocal) {
    ...
    bool has_pistols = strcmp(extractedWeaponName, CS_XOR("hkp2000")) == 0 || strcmp(extractedWeaponName, CS_XOR("usp")) == 0 || strcmp(extractedWeaponName, CS_XOR("glock")) == 0;
    ...
    if (has_awp) {
        ...
        rage_data.rage_silent_mode = C_GET_ARRAY(int, 7, Vars.rage_silent_mode, 6);
        return;
    }
    else if (has_scout) {
        ...
        rage_data.rage_silent_mode = C_GET_ARRAY(int, 7, Vars.rage_silent_mode, 5);

        return;

    }
    else  if (data->m_WeaponType() == WEAPONTYPE_PISTOL ) {
        if (has_heavy_pistols) {
            ...
            rage_data.rage_silent_mode = C_GET_ARRAY(int, 7, Vars.rage_silent_mode, 2);
            ...
            rage_data.rage_silent_mode = C_GET_ARRAY(int, 7, Vars.rage_silent_mode, 1);
            return;
        }
    }
        ...
        rage_data.rage_silent_mode = C_GET_ARRAY(int, 7, Vars.rage_silent_mode, 3);
        return;
        ...
        rage_data.rage_silent_mode = C_GET_ARRAY(int, 7, Vars.rage_silent_mode, 4);
        return;
