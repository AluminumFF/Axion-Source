...
namespace F::RAGE
{
	...
	public:
		...
		void ScanPoint(C_CSPlayerPawn* pLocal, CUserCmd* cmd, C_CSPlayerPawn* record, scan_data& data,  bool optimized);
...
namespace F::LAGCOMP
{
	...
	public:
		...
		struct LagCompSubTickData {
			int best_tick = 0;
			float best_tick_fraction = 0;
			msg_t msg = msg_t::none;
			request_t request = request_t::empty;
			QAngle_t hitbox_position = QAngle_t(0, 0, 0);
			void reset() {
				hitbox_position.Reset();
				best_tick = 0;
				best_tick_fraction = 0.f;
				msg = msg_t::none;
				request = request_t::empty;
